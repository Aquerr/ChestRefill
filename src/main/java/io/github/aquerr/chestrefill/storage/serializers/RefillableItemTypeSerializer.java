...
//    private static final TypeToken<ItemStack> ITEM_STACK_TYPE_TOKEN = TypeToken.of(ItemStack.class);
    ...
    @Override
    public RefillableItem deserialize(TypeToken<?> type, ConfigurationNode value) throws ObjectMappingException
    {
        ...
        // Validate the item.
        if (snapshot.getItem().getType() == ItemTypes.NONE)
        {
            // don't bother
    ...
    @Override
    public void serialize(TypeToken<?> type, RefillableItem obj, ConfigurationNode value) throws ObjectMappingException
    {
        if (obj == null)
            return;
